...
/**
  * FIXME 管理一个Driver执行，包括在Driver失败时自动重启Driver，目前这种方式仅仅使用于standalone集群部署模式
...
private[spark] class DriverRunner(
    ...
    new Thread("DriverRunner for " + driverId) {
      override def run() {
        try {
          ...
          // 构建ProcessBuilder，传入Driver的启动命令，需要的内存大小信息
        ...
        // 这个Driverrunner向它所属的worker的actor发送一个DriverStateChanged事件
  ...
  /**
    * fixme 将用户jar包下载到提供的目录（createWorkingDirectory方法创建的目录）
    * 并返回它在worker本地的路径，如果下载jar包的过程中出现异常，那么会抛出exception异常
