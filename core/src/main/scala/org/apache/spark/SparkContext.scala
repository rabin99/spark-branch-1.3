...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  /**
    ...
    *
    *                                  from a list of input files or
    *                                  InputFormats for the application.
    *
    */
  @DeveloperApi
  ...
  // FIXME 最重要的入口代码：Create and start the scheduler，调用createTaskScheduler()
  private[spark] var (schedulerBackend, taskScheduler) =
    SparkContext.createTaskScheduler(this, master)
  private val heartbeatReceiver = env.actorSystem.actorOf(
    Props(new HeartbeatReceiver(this, taskScheduler)), "HeartbeatReceiver")

  // FIXME 申明一个volatitla的变量，下划线意思同null
  ...
  // FIXME spark metrics 允许用户将spark的metrics统计指标，可自定义sink来指定metrics指标要上报的位置
  ...
  /** FIXME hadoopFile会创建一个hadoopRDD,key是offset，value是文本行
    * map方法去除key，只保留文本行
  ...
  def hadoopFile[K, V, F <: InputFormat[K, V]](path: String)
                                              (implicit km: ClassTag[K],
                                               vm: ClassTag[V], fm: ClassTag[F]): RDD[(K, V)] =
  ...
  def runJob[T, U: ClassTag](
    ...
    // FIXME 调用SparkContext之前初始化创建DAGScheduler的runJob()方法
...
object SparkContext extends Logging {
  ...
  /**
    * The active, fully-constructed SparkContext.
    *   If no SparkContext is active, then this is `None`.
  ...
  /**
    * FIXME 对上面定义的属性private[spark] var (schedulerBackend, taskScheduler)进行初始化
    * FIXME schedulerBackend 1，内部new Appclient负责和Master进行通信，用于注册Application，2.分配计算资源executor，3.负责发送Task到executor上执行
    * FIXME TaskScheduler 接口，是一个低层调度器，向Master注册Appliction（通过schedulerBackend实现）并负责TaskSet的任务提交到executor（task分配算法）
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      // FIXME 如果是spark standalone模式，那么对应返回(SparkDeploySchedulerBackend,TaskSchedulerImpl)
      ...
      case LOCAL_CLUSTER_REGEX(numSlaves, coresPerSlave, memoryPerSlave) =>
        ...
        val backend = new SparkDeploySchedulerBackend(scheduler, sc, masterUrls)
        // FIXME sheduler 初始化backend,TaskScheduler管理SchedulerBackend
        scheduler.initialize(backend)
      ...
      // FIXME 如果是yarn模式I，那么spark自定义了额外的类来处理，在yarn包下
      // FIXME yarn-cluser对应YarnClusterSchedulerBackend,YarnClusterScheduler
      case "yarn-standalone" | "yarn-cluster" =>
        ...
        scheduler.initialize(backend)
        (backend, scheduler)
      // FIXME Yarn-client 对应 YarnClientSchedulerBackend,YarnScheduler
      case "yarn-client" =>
        val scheduler = try {
          val clazz =
