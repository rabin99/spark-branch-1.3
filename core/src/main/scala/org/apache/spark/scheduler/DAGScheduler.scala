...
/** FIXME
  * 实现了面向stage的调度机制的高层次的调度层。它会为每个job计算stage的DAG，
  * 追踪RDD和stage的输出是否被物化（写入磁盘或者内存等地方）
  * 并且寻找一个最少消耗（最优、最小）调度机制来运行job。
  * 它会将stage作为TaskSets提交到底层的TaskSchedulerImpl上，来在集群上运行它们（task）
  *
  * 除了处理stage的DAG，它还负责决定运行每个task的最佳位置，基于当前的缓存状态，将这些最佳位置交给底层的TaskSchedulerImp。
  * 此外，它会处理由于shuffle输出文件丢失导致的失败，在这种情况下，旧的stage可能就会被重新提交。
  * 一个stage内部的失败，如果不是由于shuffle文件丢失锁导致的，会被TaskScheduler处理，它会多次重试每一个task，
  * 直到最后实在不行了，才会去取消整个stage。
  *
...
class DAGScheduler(
  ...
  private def newStage(
                        ...
                        callSite: CallSite)
  : Stage = {
    // fixme 通过rdd以及jobid，获取父stage
    val parentStages = getParentStages(rdd, jobId)
    // fixme stageid是个AtomicInteger
  ...
  /**
    * FIXME 获取父stage关键算法：获取stage父stage
  ...
  private def getMissingParentStages(stage: Stage): List[Stage] = {
    ...
    // We are manually maintaining a stack here to prevent StackOverflowError
    // caused by recursively visiting
    // 栈，先入后出，这样先得到的stage被推入栈底，栈顶的肯定是第一个stage0
    val waitingForVisit = new Stack[RDD[_]]
    ...
    def visit(rdd: RDD[_]) {
      if (!visited(rdd)) {
        ...
        if (getCacheLocs(rdd).contains(Nil)) {
          ...
          // 遍历Rdd的依赖，其实对于每一种有shuffle的操作，比如阿groupByKey、reduceByKey、countByKey
  ...
  private[scheduler] def handleJobSubmitted(jobId: Int,
    ...
    // 第一步，使用出发job的最后一个rdd，创建finalStage
    var finalStage: Stage = null
    try {
      // New stage creation may throw an exception if, for example, jobs are run on a
      // HadoopRDD whose underlying HDFS files have been deleted.
      // fixme 关键方法：创建一个stage对象，并且将stage加入DAGScheduler内部的缓存中
    ...
    if (finalStage != null) {
      // fixme 第二步，用finalStage创建一个job
      // fixme 就是说，用这个job的最后一个stage，当然就是我们的finalStage了
      ...
      // fixme 判断是不是可以在本地运行，一般用于local模式，或其他测试
      ...
      if (shouldRunLocally) {
        ...
        //  fixme 第四步，使用submitStage()方法提交finalStage
        // 这个方法的调用，其实会导致第一个stage提交
        // 并且导致其他所有stage，都给放入waitingStages队列里
        /** stage划分算法总结
          * 1. 从finalStage倒退
  ...
  /**
    * FIXME 提交stage的方法，其实就是stage划分的入口
    * stage划分算法，其实是由submitStage()方法自身递归，在方法内部使用getMissingParentStages()获取当前stage的父stage
    * 直到碰到missing ==Nil说明是stage0了，然后调用submitMissingTask提交
    *
  ...
  private def submitStage(stage: Stage) {
    ...
    if (jobId.isDefined) {
      ...
      // 如果stage不是等待、正在执行、失败的stage，则开始调度
      if (!waitingStages(stage) && !runningStages(stage) && !failedStages(stage)) {
        ...
        if (missing == Nil) {
          ...
          // 并将当前stage放入waitingStages等待执行的stage的队列中
          waitingStages += stage
        }
      }
    } else {
  ...
  /** fixme 提交stage，为stage创建一批task，task数量与partitino数量相同
